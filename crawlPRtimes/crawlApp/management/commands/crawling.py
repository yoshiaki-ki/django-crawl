...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        for article in article_list:
            ...
            article_in["company_link"] = "https://prtimes.jp" + company_link
        ...
        all_company_list = []
        for company in all_company_obj:
            all_company_list.append(company.company_name)

        ## 記事の保存  ## 12/23 このままではDBに重複して入ってしまう
        for article in data_list:
            ...
            if article_title not in all_article_titles:
                a = Article(company_id= CompanyInfo.objects.get(id=1), title=article_title, article_url=article_url, release_time=article_release_time)
                a.save()
            if company not in all_company_list:
                c = CompanyInfo(company_name=company, PRtimes_URL=company_url)
                c.save()
                
        print("Article記事　保存完了")
